model {
  ...
  for (i in 1:t){
    ...
    BScyst.d2H[i] = BScyst.slope.lw.2H * rprx.L.2H[i] + BScyst.slope.diet.2H * BSdiet.d2H[i] + BScyst.inc.2H
    
    BScyst.d18O[i] = BScyst.slope.lw.18O * rprx.L.18O[i] + BScyst.slope.diet.18O * BSdiet.d18O[i] + BScyst.inc.18O
  ...
  prx.L.v = LV * f.m.ro
  #fraction of lake water that is mixed with runoff
  f.m.ro ~ dbeta(4,40) T(0,0.3) # this fraction should be small, so a cap of 0.3 is set for this parameter
  ...
  for (i in 2:t){
    ...
    # E.rate[i] =  2.909* nsws * (LA[i]*1e6)^-0.05 * (S.coeff[i] - rh) * Vpfs[i]/(2.501-0.002361*LST[i])/1e6 * n.d.evap*1000
  ...
  n.d.evap.int = 120  #prior: 120 days +- 10 days
  ...
  #f: fraction of advected air over lake, Tanganyika is set at 0.3, GSL is set at a similar value
  f ~ dbeta(20, 50)
